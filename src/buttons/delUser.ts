...
export default new Button(
    ...
    async (
        ...
        collector.on('collect', async (m: Message): Promise<any> => {
            ...
            Promise.all([
                await channel.permissionOverwrites.create(member.id, {Connect: false}),
                await button.editReply({content: `Участник ${member.toString()} больше **не может** присоеденяться к **вашей** комнате`})
            ])
        })
    }
)