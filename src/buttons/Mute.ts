import { ButtonInteraction, Message, OverwriteType, VoiceChannel } from 'discord.js'
...
export default new Button(
    ...
    async (
        ...
        collector.on('collect', async (m: Message): Promise<any> => {
            ...
            Promise.all([
                await channel.permissionOverwrites.create(member.id, {Speak: false}),
                await button.editReply({content: `Вы **закрыли** ротик участнику ${member.toString()}`})
            ])
        })
    }
)