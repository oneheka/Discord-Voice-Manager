...
export default new Event(
    ...
    async (bot: Core, interaction: Interaction) => {
        ...
        if(interaction.isButton() && interaction.channel.id == config.channels.text) {
            ...
            if(!room || !author?.voice || !author?.voice?.channel) return button.reply({content: 'Вы находитесь **не в голосовом** канале', ephemeral: true})
            ...
            if(bot.buttons.has(button.customId)) {
                await bot.buttons.get(button.customId)(button, room, channel)
                return
            } else {
